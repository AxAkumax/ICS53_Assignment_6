...
void* producer(void* arg) {
    ...
    for (int i = 0; i < items_per_producer; ++i) {
        ...
        //add the tail (pointer) to move onto the next slot to produced
...
void* consumer(void* arg) {
    ...
    for (int i = 0; i < items_per_producer * total_producers / total_consumers; ++i) {
        ...
        //add the head (pointer) to move onto the next slot to be consumed
...
int main(int argc, char* argv[]) {
    ...
    // Wait for all producer threads to finish (threads are non-detached hence join is used)
    for (int i = 0; i < total_producers; ++i) {
